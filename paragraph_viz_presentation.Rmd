---
title: "paragraph_viz presentation"
...
beta <- predict(zeroEmbeddings, theModels[["paragraphModel"]], nameLLM)
print(beta)
shapleyPredictions <- getShapleyPredictions(normalizedOutput, beta$predicted_value)
```
...
plot <- generateDocument(shapleyPredictions, toPredEmbedShapley, data[paragraphs_to_plot, scale_total], nameLLM, limits = c(0, 27), shapley = TRUE, filePath = paste0(model_folder_path, "/plot_paragraphs_", paragraphs_to_plot[1], "-", paragraphs_to_plot[length(paragraphs_to_plot)], "-shapley",".html"))
plot
```
