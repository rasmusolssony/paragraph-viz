...
data <- data[1:11,c("harmonytexts", "hilstotal")]
```
...
#print(as.vector(data[2:11, "hilstotal"]))
```
```{r Separate sents, echo=TRUE}

#toTrain <- addSentenceEmbeddings(embed1, tokenizer, nameLLM,
#                      includeCLSSEP="both" # Keep the CLS and SEP special token as well.
#)
```
```{r Separate sents, echo=TRUE}
#temp <- getTrainableTokens(toTrain, data[2:11, "hilstotal"], tokenizer, nameLLM)
```
...
theModels <- trainLanguageModel(embed1, # embedding
                       data[2:11, "hilstotal"], # scale values, target
...
output <- langPred(toPredEmbed, theModels, "all", tokenizer, nameLLM)
